...
final class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        gameCamera.position = CGPoint(
            x: max(size.width * 0.5, player.position.x),
            y: size.height * 0.5
        )
    }
    ...
    private func layoutScene() {
        ...
        // Keep camera vertically centered on the game space.
        gameCamera.position.y = max(size.height * 0.5, 210)
    }
    ...
    private func updateCamera(_ dt: CGFloat) {
        ...
        let lookAhead = (player.physicsBody?.velocity.dx ?? 0) * 0.18

        var targetX = player.position.x + lookAhead
        targetX = max(halfScreen, targetX)
        targetX = min(worldWidth - halfScreen, targetX)

        let smoothing: CGFloat = 5.2
        gameCamera.position.x +=
            (targetX - gameCamera.position.x) * min(1, smoothing * dt)
    }
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        if isPlayerGroundContact(contact) {
            ...
            if contact.contactNormal.dy > 0.35 ||
