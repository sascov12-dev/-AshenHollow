        ...
        for touch in touches {
            ...
            if leftButton.contains(point) {
                ...
                animateButton(rightButton, pressed: true)
                        ...
                        hypot(point.x - jumpButton.position.x,
                              point.y - jumpButton.position.y) <= 85 {
                jumpTouches.insert(id)
                jumpHeld = true
                jumpBufferTimer = jumpBufferDuration
                didCutJump = false

                // Direct launch on press. This bypasses grounded/contact state entirely,
                // so the jump cannot be blocked by a bad contact callback.
